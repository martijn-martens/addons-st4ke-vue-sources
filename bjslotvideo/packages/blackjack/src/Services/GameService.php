...
class GameService extends ParentGameService
{
    ...
    public function deal($params): GameService
    {
        ...
        $gameable->dealer_hand = $this->drawCards([1, 3]);
...
//        ---------------------------------------------------------------------
        ...
        if ($gameable->dealer_hand[0][1] != 'A' || $this->getUserAccountBalance() < $gameable->bet / 2) {
            // if player has blackjack
            if ($gameable->player_score == 21 && $gameable->dealer_score != 21) {
                $gameable->player_blackjack = TRUE;
                $gameable->win = $gameable->bet * 3 / 2 + $gameable->bet; // blackjack pays 3/2
    ...
    public function stand(): GameService
    {
        ...
        while ($this->calculateScore($gameable->dealer_hand) < 17) {
            ...
            $gameable->dealer_hand = array_merge(
                ...
                $this->drawCards([count($gameable->player_hand) + count($gameable->dealer_hand)])
            );
        }
    ...
    public function insurance($insurance): GameService
    {
        ...
        if ($insurance) {
            ...
            // if dealer has blackjack and player does not
                ...
                $gameable->dealer_blackjack = TRUE;
                $gameable->insurance_win = $gameable->insurance_bet * 2/1 + $gameable->insurance_bet; //insurance pays 2:1
                $gameable->win = 0;
    ...
    public function split(): GameService
    {
        ...
        // split hands
        $gameable->player_hand = [$playerHand[0], $this->drawCards([4])[0]]; // get 1 new card, for the first split hand
        $gameable->player_score = $this->calculateScore($gameable->player_hand);
    ...
    public function splitStand(int $hand, bool $save = TRUE): GameService
    {
        ...
        if ($hand == 1) {
            ...
            $gameable->player_hand2 = array_merge($gameable->player_hand2, $this->drawCards([count($gameable->player_hand) + count($gameable->dealer_hand) + 1]));
            $gameable->player_score2 = $this->calculateScore($gameable->player_hand2);
            if ($gameable->player_score2 < 21) {
        ...
        if ($hand == 2 || $gameable->player_score2 == 21) {
            while ($this->calculateScore($gameable->dealer_hand) < 17) {
                ...
                $gameable->dealer_hand = array_merge(
                    $gameable->dealer_hand,
                    $this->drawCards([count($gameable->player_hand) + count($gameable->player_hand2) + count($gameable->dealer_hand)])
                );
            }
    ...
    public function splitHit(int $hand): GameService
    {
        ...
        // get new card
        $newCard = $this->drawCards([count($gameable->player_hand) + count($gameable->player_hand2) + count($gameable->dealer_hand)]);
    ...
    private function addActions($target, array $actions): GameService
    {
