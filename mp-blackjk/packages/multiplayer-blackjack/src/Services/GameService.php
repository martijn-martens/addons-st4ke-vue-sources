...
//        $gameable->deck = array_merge(['DT','ST','HT','CT'], array_slice($gameable->deck, 4));
     ...
     * @return Model
     */
    private function play(Model $gameable): Model
    {
        $hands = $gameable->hands;

        $offset = $hands->count() == 0 ? 0 : $hands->count() * 2;
        $cards = $this->drawCards([$offset, $offset + 1]);
     ...
     * @return Model
     */
    private function stand(Model $gameable): Model
    {
        $user = $this->getUser();
        $time = time();
        $i = 0;
     ...
     * @return Model
     */
    private function hit(Model $gameable): Model
    {
        $user = $this->getUser();
        $hands = $gameable->hands;
        ...
        $hand['cards'] = array_merge($hand['cards'], $this->drawCards([$drawnCardsCount]));
     ...
     * @return Model
     */
    private function complete(Model $gameable): Model
    {
        $room = $this->getRoom();
        $hands = $gameable->hands;
     ...
     * @return Model
     */
    private function cancel(Model $gameable): Model
    {
        $room = $this->getRoom();
        $hands = $gameable->hands;
    ...
    private function getGameEvent(string $action): array
    {
        $gameable = $this->getGameable();
